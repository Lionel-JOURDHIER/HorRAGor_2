...
@router.post(
    "/login", response_model=AuthResponse, responses={401: {"model": ErrorResponse}}
)
...
@router.post(
    "/refresh", response_model=Token, responses={401: {"model": ErrorResponse}}
)
async def refresh_token(token_data: TokenRefresh, db: Session = Depends(get_db)):
    ...
    if not user or not user.is_active:
        logger.warning(
            f"Utilisateur introuvable ou inactif : {refresh_token_obj.user_id}"
        )
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
@router.get(
    "/me", response_model=UserResponse, responses={401: {"model": ErrorResponse}}
)
