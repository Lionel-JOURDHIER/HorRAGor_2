        ...
        with col2:
            ...
            if movie.get("genres"):
                ...
                if isinstance(genres, list):
                    ...
                    st.markdown('<strong style="color: #ff4757; font-size: 1.1em;">🎭 Genres :</strong>', unsafe_allow_html=True)
...
def display_movie_list(movies: List[Dict[str, Any]], title: str = "🎬 Films recommandés") -> None:
    ...
    for idx, movie in enumerate(movies, 1):
        ...
        with st.expander(f"{medal} {titre} ({annee}) - ⭐ {score}/10", expanded=(idx == 1)):
...
def create_filters_sidebar(api_url: str) -> Dict[str, Any]:
    ...
    with st.sidebar:
        ...
        duree_min_h = f"{duree_range[0]//60}h{duree_range[0]%60:02d}" if duree_range[0] >= 60 else f"{duree_range[0]}min"
        duree_max_h = f"{duree_range[1]//60}h{duree_range[1]%60:02d}" if duree_range[1] >= 60 else f"{duree_range[1]}min"
        ...
        nb_filtres = sum([
            ...
            1 if (filters.get("date_sortie_min") != 1900 or filters.get("date_sortie_max") != 2026) else 0,
            1 if filters.get("score_tmdb_min") > 0 else 0,
            1 if (filters.get("duree_min") != 1 or filters.get("duree_max") != 685) else 0
        ])
...
def display_agent_status(status: Dict[str, Any]) -> None:
    ...
    st.markdown('<div style="background: rgba(255, 107, 107, 0.05); padding: 15px; border-radius: 10px; margin: 10px 0;">', unsafe_allow_html=True)
