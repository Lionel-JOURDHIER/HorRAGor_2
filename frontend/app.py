...
def display_header():
    ...
    with col2:
        st.markdown(
            '<h1 style="text-align: center; color: #ff4757; font-size: 4.5em; margin-bottom: 0; font-weight: 900; text-shadow: 0 0 25px rgba(255, 71, 87, 0.6);">🎬 HorRAGor</h1>',
            unsafe_allow_html=True,
        )
        st.markdown(
            "<h3 style=\"text-align: center; color: #ffffff; opacity: 0.95; font-weight: 600; text-shadow: 0 2px 10px rgba(0,0,0,0.5);\">Votre guide intelligent dans l'univers de l'horreur</h3>",
            unsafe_allow_html=True,
        )
...
def check_api_status():
    ...
    if status.get("status") == "error":
        ...
        st.error(
            "⚠️ L'API n'est pas accessible. Vérifiez que le serveur FastAPI est démarré."
        )
...
def main():
    ...
    if not st.session_state.messages:
        ...
        with col1:
            ...
            if st.button(
                "🎬 Meilleurs films de John Carpenter", use_container_width=True
            ):
                st.session_state.preset_question = (
