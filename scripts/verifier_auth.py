...
def test_auth_system():
    ...
    try:
        response = requests.post(
            f"{API_URL}/auth/register", json=register_data, timeout=10
        )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"   ❌ Échec de la récupération du profil : {response.status_code}\n"
            )
    except Exception as e:
    ...
    try:
        response = requests.post(
            f"{API_URL}/auth/refresh", json={"refresh_token": refresh_token}, timeout=10
        )
